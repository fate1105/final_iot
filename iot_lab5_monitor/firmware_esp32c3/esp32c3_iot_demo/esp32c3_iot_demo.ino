...
// WiFi Configuration
const char *WIFI_SSID = "Rose";       // TDMU WiFi (no password)
const char *WIFI_PASSWORD = "thuongtha@Fpt"; // Empty - no password

// MQTT Broker Configuration
const char *MQTT_HOST = "192.168.1.25"; // Your computer's IP on TDMU network
...
#define DHT_TYPE DHT22 // DHT11 sensor type

#define LED_PIN 5 // Built-in LED (Light control)

// L298N Motor Driver pins
#define MOTOR_IN1 6  // L298N IN1 (changed from 8 to avoid conflict)
#define MOTOR_IN2 7  // L298N IN2 (changed from 9)
...
void initGPIO()
{
    ...
    // Setup PWM for motor speed control (ESP32 Arduino Core 3.x)
...
void reconnectWiFi() {
    if (WiFi.status() != WL_CONNECTED) {
        Serial.println("⚠️  WiFi disconnected, reconnecting...");
        WiFi.disconnect();
        delay(100);  // Đợi một chút trước khi kết nối lại
        WiFi.begin(WIFI_SSID, WIFI_PASSWORD);  // Kết nối lại
        int attempts = 0;
        while (WiFi.status() != WL_CONNECTED && attempts < 20) {
            delay(500);
            Serial.print(".");
